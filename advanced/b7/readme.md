# Урок B7. Базы данных: сырой SQL, async-драйверы и ORM (SQLAlchemy + Pydantic)

В B4 мы подняли API на FastAPI, но данные брали из списка в памяти — «вместо базы».
...
Урок опирается на теорию **event loop и GIL** из B4 — если подзабыл, почему БД-вызовы
обязаны быть асинхронными, перечитай третью часть B4.
...
├── task.md                     — практическое задание (мини-библиотека)
...
Из B4: FastAPI крутится на **одном потоке** в event loop. Если внутри `async def`
...
В B4 мы разделяли **модель** (внутренняя сущность) и **схему** (что видит клиент).
...
[TestTask_260329](https://github.com/JustChasti/TestTask_260329) (FastAPI + слои из
B4 + БД). Как там разложена БД-часть:
...
Так замыкается цепочка уроков: **B4** дал слои и `async def`, **B7** насаживает на
...
Практическое задание — в файле [task.md](task.md): мини-библиотека
